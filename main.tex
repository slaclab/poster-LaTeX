...
\footercontent{ \hfill IEEE ISCAS 2024 - 19$^{\text{th}}$ to 22$^{\text{th}}$ May, Singapore \hfill}
...
\begin{column}{\colwidth}
    ...
    This section provides a comparison between the proposed switching scheme of \Cref{fig:scheme} and state-of-the-art 10-bit SAR ADC solutions. \label{sec:comp}\Cref{tab:comp} summarizes the performance metrics, focusing on recent techniques that exhibit exceptionally low energy switching: $E/(C V^2)<32$. The comparison specifically emphasizes architectures utilizing binary weighted configurations without the use of bridge capacitor techniques and, similar to other schemes considered in this analysis, the parasitic capacitors and energy consumed during the reset phase are not taken into account. The conventional SAR is used as the standard reference to assess energy and area efficiency for all techniques. It is worth mentioning that the switching energy is influenced by the input analog value. While \Cref{tab:comp} displays the expected switching energy of multiple switchig techniques assuming all input values are equally probable, \Cref{fig:ed} shows $E(D)$, i.e., the energy for each digital output code.
    ...
    \end{table}
     ...
     \begin{figure}[t] \centering
       \begin{tikzpicture}
    	   % red, blue, green, darkgray, orange, purple, brown, black, gray, olive, teal
        \begin{axis}[width=16cm, height=10cm, legend style={at={(0,1.0)}, legend columns=6, anchor=south west}, ylabel={\textit{Switching Energy ($E / C V^2$)}}, xlabel={\textit{Output Code ($D$)}}, grid=major, enlarge x limits=false, every axis y label/.style={at={(ticklabel cs:0.5)},rotate=90,anchor=near ticklabel}, mark size = 1.5pt ]
            ...
            \addplot[mark=none, mark size = .75pt, color=black, line width=1.75pt] table [each nth point=7, x=D, y={S3}, col sep=comma] {\datasetED}; \addlegendentry{\textbf{\scriptsize{This work}}}
